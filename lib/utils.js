...
export const makeCrossQuery = ({ g1, e1, g2, e2 }) => {
  ...
  const seedParent1Clause = `(${seedParent1.map(makeClause).join(" and ")})`;
  const pollenParent2Clause = `(${pollenParent2
    .map(makeClause)
    .join(" and ")})`;
  const combo1 = [seedParent1Clause, pollenParent2Clause].join(" and ");

  const seedParent2Clause = `(${seedParent2.map(makeClause).join(" and ")})`;
  const pollenParent1Clause = `(${pollenParent1
    .map(makeClause)
    .join(" and ")})`;
  const combo2 = [seedParent2Clause, pollenParent1Clause].join(" and ");
